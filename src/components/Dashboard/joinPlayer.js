...
class Joinplayer  extends Component {
    ...
    joinPlayerIntoGame() {
        ...
        var propState = this;
        firebase.database().ref('games/'+this.props.gameId+'/players/' + playerId).set(playerData).then(function(snapshot) {
        ...
        return (
            <Aux>
                {this.isPlayerAlreadyAdded(this.props.playerId) ?
                    <button className="btn-round btn-grey fr" disabled>Delete</button> :
                    <button className="btn-round btn-black fr" onClick={this.joinPlayerIntoGame}>Add</button>
                }
            </Aux>
        )
    }
